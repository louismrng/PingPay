...
/// <summary>
/// Handles Telegram bot commands and orchestrates responses.
/// Uses the same command parsing and business logic as WhatsApp.
/// </summary>
public class TelegramBotService : ITelegramBotService
{
    ...
    {
        _logger.LogInformation("Processing Telegram message from {UserId}", userId);
        ...
        // Get or create user - for Telegram, userId is the chat ID
        var user = await _userRepository.GetByPhoneNumberAsync(userId, ct);

        return command.Type switch
        {
            CommandType.Help => Help(),
            CommandType.Register => await RegisterAsync(userId, user, ct),
        ...
        CancellationToken ct)
    {
        ...
        // Create new user - use userId as phone number for now
        var user = await _userRepository.CreateAsync(new User
        {
            PhoneNumber = userId,
            IsActive = true,
            IsVerified = true // Verified via Telegram
        }, ct);
        ...
        _logger.LogInformation("New user registered via Telegram: {UserId}", user.Id);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Telegram transfer initiated: {TxId} from {Sender} to {Recipient}",
        ...
        {
            _logger.LogError(ex, "Telegram transfer failed");
